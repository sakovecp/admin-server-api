...
> Примітка: якщо API запущено в докері — конфігурації та HTML записуються в директорії `docker/nginx/*`, які проброшені в `docker-compose.yml`. Якщо API запущено на хості скрипти намагаються використовувати системний nginx.
...
## Скільки зайняло часу / зауваження (від автора README)
